...
<template>
    ...
    <div class="min-h-screen bg-white font-black antialiased text-zinc-900 selection:bg-[#10B981] selection:text-white font-['Inter']">
        ...
        <nav :class="[isScrolled ? 'bg-white/95 backdrop-blur-md py-3 shadow-sm border-b border-zinc-100' : 'bg-white py-4 border-b border-zinc-50']" 
            ...
            <div class="flex items-center gap-10">
                <Link :href="route('welcome')" class="relative flex items-center group/nav font-black">
                    <div class="bg-zinc-900 text-white h-7 px-2 flex items-center justify-center transition-all duration-500 group-hover/nav:md:px-4 text-sm font-black italic font-black">
                        A<span class="max-w-0 overflow-hidden whitespace-nowrap text-[8px] tracking-[0.4em] transition-all duration-500 group-hover/nav:max-w-[100px] group-hover/nav:ml-2 uppercase italic hidden md:block font-black">STUDIO</span>
                    </div>
                    <div class="flex flex-col ml-3 leading-none text-left font-black font-black">
                        <span class="text-sm font-black tracking-[-0.05em] uppercase font-['Plus_Jakarta_Sans'] font-black">ADONIS</span>
                        <span class="text-[5px] tracking-[0.45em] text-zinc-400 uppercase mt-0.5 italic font-bold font-black">Philippines</span>
                    </div>
                </Link>

                <div class="hidden md:flex items-center gap-8 border-l border-zinc-100 pl-10 font-black">
                    <Link :href="route('archive.index')" class="text-[9px] uppercase tracking-[0.3em] text-zinc-400 hover:text-black transition-colors font-black">Archive</Link>
                </div>
            </div>

            <div class="flex items-center gap-10">
                <Link :href="route('cart.index')" class="hidden md:flex items-center gap-2 group/cart text-zinc-400 hover:text-[#10B981] transition-colors relative">
                    ...
                    <span v-if="cartCount > 0" class="absolute -top-1 -right-2 text-[6px] bg-[#10B981] text-white px-1.5 py-0.5 rounded-full border border-white font-black">{{ cartCount }}</span>
                </Link>
        ...
        <main class="mt-24 md:mt-32 max-w-[1200px] mx-auto w-full px-4 md:px-6 pb-28 text-left font-black font-black">
            ...
            <div class="flex flex-col gap-0.5 mb-8 border-b border-zinc-900 pb-4 font-black">
                <span class="text-[8px] text-[#10B981] tracking-[0.6em] uppercase font-bold italic font-black">Step 01: Acquisition</span>
                <h1 class="text-2xl md:text-3xl font-black uppercase tracking-tighter leading-none italic font-['Plus_Jakarta_Sans'] text-zinc-900 font-black font-black">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start font-black">
                ...
                <div class="lg:col-span-7 flex flex-col md:flex-row gap-4 font-black">
                    <div v-if="product.images?.length > 0" 
                         class="flex md:flex-col gap-2 overflow-x-auto md:overflow-y-auto no-scrollbar shrink-0 md:w-16 md:max-h-[480px] font-black font-black">
                        
                        <button @click="activeImage = product.image_path"
                                class="w-14 h-18 bg-zinc-50 border transition-all overflow-hidden shrink-0 rounded-lg font-black font-black"
                    ...
                    <div class="flex-1 bg-zinc-50 border border-zinc-100 aspect-[3/4] overflow-hidden group/img relative rounded-xl max-w-[420px] mx-auto lg:mx-0 shadow-sm font-black">
                        <div v-if="isFullySoldOut" class="absolute inset-0 z-20 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm font-black font-black">
                            <div class="border border-white/50 px-5 py-2.5 transform -rotate-12 font-black font-black font-black">
                                <span class="text-white text-sm uppercase font-black italic tracking-[0.4em] font-black font-black">Sold Out</span>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-5 space-y-8 lg:sticky lg:top-40 font-black">
                    <div class="space-y-3 font-black">
                        ...
                        <h2 class="text-3xl md:text-4xl font-black uppercase tracking-tighter leading-[0.9] italic font-['Plus_Jakarta_Sans'] font-black">{{ product.name }}</h2>
                    ...
                    <div class="space-y-6 pt-6 border-t border-zinc-100 font-black font-black">
                        <div class="space-y-3 font-black font-black">
                            <label class="text-[7px] uppercase tracking-[0.5em] text-zinc-400 font-black font-black">Configuration Selection</label>
                            <div class="grid grid-cols-4 gap-2 font-black">
                                <button v-for="v in product.variants" :key="v.id" @click="form.variant_id = v.id"
                                        ...
                                        class="h-9 border text-[8px] font-black uppercase transition-all italic rounded-lg font-black font-black">
                                    {{ v.size }}
                                </button>
                            </div>
                        </div>

                        <div class="pt-6 space-y-5 font-black font-black font-black">
                            <div class="flex justify-between items-end font-black font-black">
                                <span class="text-[8px] uppercase tracking-[0.4em] text-zinc-400 italic font-black font-black font-black">Valuation</span>
                                <span class="text-2xl font-black tracking-tighter italic font-['Plus_Jakarta_Sans'] font-black">
                            ...
                            <button @click="addToCart" :disabled="isSelectedVariantSoldOut || form.processing" 
                                    ...
                                    :class="[isSelectedVariantSoldOut ? 'bg-zinc-100 text-zinc-400 font-black' : 'bg-black text-white hover:bg-[#10B981] font-black']">
                                <span>{{ isSelectedVariantSoldOut ? 'Depleted' : (form.processing ? 'Syncing...' : 'Acquire Piece') }}</span>
                                <svg v-if="!isSelectedVariantSoldOut" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="group-hover:translate-x-1 transition-transform font-black"><path d="M5 12h14M12 5l7 7-7 7 font-black"/></svg>
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <nav class="md:hidden fixed bottom-0 w-full bg-white border-t border-zinc-100 z-[120] flex items-center justify-around py-3 px-4 shadow-[0_-15px_40px_rgba(0,0,0,0.08)] font-black">
            <Link :href="route('welcome')" class="flex flex-col items-center gap-1 group font-black">
                ...
                <span class="text-[7px] uppercase tracking-widest font-bold font-black font-black" :class="[route().current('welcome') ? 'text-zinc-900' : 'text-zinc-300']">Home</span>
            </Link>
            <Link :href="route('archive.index')" class="flex flex-col items-center gap-1 group font-black">
                ...
                <span class="text-[7px] uppercase tracking-widest font-bold font-black font-black" :class="[route().current('archive.index') ? 'text-zinc-900' : 'text-zinc-300']">Shop</span>
            </Link>
            <Link :href="route('cart.index')" class="relative flex flex-col items-center gap-1 group font-black">
                ...
                <span v-if="cartCount > 0" class="absolute -top-1.5 right-0 text-[6px] bg-[#10B981] text-white px-1.5 py-0.5 rounded-full border border-white font-black">{{ cartCount }}</span>
                <span class="text-[7px] uppercase tracking-widest font-bold font-black font-black" :class="[route().current('cart.index') ? 'text-[#10B981]' : 'text-zinc-300']">Bag</span>
            </Link>
            <Link :href="route('orders.index')" class="flex flex-col items-center gap-1 group font-black">
                ...
                <span class="text-[7px] uppercase tracking-widest font-bold font-black font-black" :class="[route().current('orders.*') ? 'text-zinc-900' : 'text-zinc-300']">Orders</span>
            </Link>
        </nav>
